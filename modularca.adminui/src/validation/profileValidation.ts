 ...
 *   ModularCA.API/Controllers/v1/Admin/AdminCertSignRequestController.cs:123 (ValidateAgainstProfile)
 *   ModularCA.API/Controllers/v1/Admin/AdminCertSignRequestController.cs:292 (ValidateFieldValue)
 *   plus the matching User* controller.
 */
...
export function validateAgainstProfileClient(
    ...
    for (const san of sans) {
        ...
        // Mirror server SanShapeValidator — only run if no upstream error.
...
function isValidIpv6(s: string): boolean {
    // Accept compressed (::), full 8-group, and IPv4-embedded forms. Reject zone-id
    // suffix (% notation) since cert SANs shouldn't carry it.
    ...
    for (const half of groups) {
        ...
        for (const p of parts) {
            // Allow embedded IPv4 in the last group.
            if (p.includes('.')) {
